...
void recvIpcMessage(uint16_t ID, uint16_t Data[])
{
    ...
    case 0x10u:
        ...
        on16.byte.B0 = (uint8_t)Data[pos++]; // 첫 번째 바이트를 하위(B0)에 저장
        on16.byte.B1 = (uint8_t)Data[pos++]; // 두 번째 바이트를 상위(B1)에 저장
...
/**
 * @brief 인코더 상태 및 데이터를 PC로 전송 (10ms 주기)
 * @details 전체 15바이트 패킷을 구성하며, 데이터 영역 길이는 11바이트(LEN 포함)입니다.
...
void sendIpcMessage1(void)
{
    ...
    Buf[pos++] = 0u;                // Buf[2]: Length (데이터 개수 11은 여기서 업데이트)
    ...
    Buf[pos++] = on16.byte.B0; // Buf[4]: Status Byte (PC는 1바이트 수신 대기)
    ...
    /* 7. CheckSum 계산 (Length인 Buf[2]부터 데이터 라인인 Buf[pos-1]까지) */
    CheckSum = 0u;
    for(i = 2u; i < pos; i++)
    {
